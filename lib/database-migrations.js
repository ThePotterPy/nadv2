...
async function initializeDatabase(db, options) {
    ...
    if (adminRecovery) {
        const recoveryMarker = await dbGet(
            db,
            'SELECT value FROM app_meta WHERE key = ?',
            [adminRecovery.marker]
        );
        if (!recoveryMarker) {
            await dbRun(db, 'BEGIN IMMEDIATE');
            try {
                const result = await dbRun(
                    ...
                    [adminRecovery.passwordHash]
                );
                if (result.changes !== 1) {
                    throw new Error('No se encontró un administrador único para recuperar');
                }
                await dbRun(db, 'DELETE FROM login_attempts');
                await dbRun(
                    db,
                    `INSERT INTO app_meta (key, value, updated_at) VALUES (?, '1', datetime('now'))`,
                    [adminRecovery.marker]
                );
                await dbRun(db, 'COMMIT');
                console.log('✅ Contraseña administrativa recuperada sin modificar el contenido');
            } catch (error) {
                await dbRun(db, 'ROLLBACK').catch(() => {});
                throw error;
            }
